...
Java_com_example_myapplication_GgwaveManager_generateAudio(JNIEnv *env, jobject thiz, jstring payload) {
    ...
    const int n = ggwave_encode(ggwaveId, c_payload, payload_len, GGWAVE_PROTOCOL_AUDIBLE_FAST, 50, nullptr, 1);

    std::vector<char> waveform(n);

    const int ret = ggwave_encode(ggwaveId, c_payload, payload_len, GGWAVE_PROTOCOL_AUDIBLE_FAST, 50, waveform.data(), 0);
